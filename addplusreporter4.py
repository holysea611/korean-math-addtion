...
class JosaCorrector:
    def __init__(self):
        ...
        # [수정] 조사가 아닌 단어(동사/형용사 활용형 등) 보호 목록 강화
        self.protected_words = [
            ...
            # 수량/범위 명사
            '이상', '이하', '이내', '이외', '미만', '초과',
            # [추가] '잇다(connect)'의 활용형 및 기타 오탐지 방지
    ...
    def get_context(self, text, start, end, window=10):
        """문맥 추출 함수 (앞뒤 window 글자 포함)"""
        s = max(0, start - window)
        e = min(len(text), end + window)
        context = text[s:e].replace('\n', ' ')
        # 강조 표시 (선택사항)
        # context = context[:start-s] + "**" + context[start-s:end-s] + "**" + context[end-s:]
        return f"...{context}..."
...
class SpellingCorrector:
    ...
    def run(self, text):
        ...
        for i, part in enumerate(parts):
            ...
            # 2. 한글/기호 조사 호응 교정 (finditer로 변경하여 문맥 추출)
            # 통합 패턴: (한글|기호)(조사)
...
st.markdown("""
**1. 수식 조사 호응:** LaTeX 수식 뒤의 조사(은/는, 이/가 등)를 교정합니다. ('이은', '이을' 등 오탐지 방지)  
**2. 한글 맞춤법:** 수학 용어, 한글/기호 뒤의 조사를 교정합니다.
""")
...
with col1:
    ...
    input_val = st.text_area("텍스트를 입력하세요:", height=600, 
                             placeholder="예: 두 점 $A$, $B$를 이은 선분은...")
